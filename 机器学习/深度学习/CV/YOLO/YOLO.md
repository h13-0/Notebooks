...
7. 找到<font color="#c00000">每个标注框的</font> `align_metric` 和 `overlap` <font color="#c00000">的最大值</font>，分别存储于 `pos_align_metrics` 和 `pos_overlaps`
8. 对 `align_metric` 进行归一化，存储于 `norm_align_metric` 
	- `norm_align_metric.shape=[batch, 8400, 1]`
	- 数学表达为：$$norm\_align\_metric=\frac{每个标注框的topk个cell的对齐分数}{每个标注框的topk个cell的最大对齐分数}\cdot 每个标注框的topk个cell的最大IoU$$
9. 使用 `target_scores` 对归一化之后的 `norm_align_metric` 进行蒙版操作，即得最终 `target_score` 
	- `target_scores.shape=[batch, 8400, cls]`
