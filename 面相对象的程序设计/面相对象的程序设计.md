...
3. `void perform(Animal &animal);` 方法允许接收比它在父类中声明的方法更一般的类型作为参数(即允许接收 `Dog` 和 `Cat` 类型)，属于<font color="#c00000">逆变</font>。
4. `perform(myDog);` 和 `perform(myCat);` ：
	1. 在多态的语言理论上的分类来看是
	2. 其多态的解析是在运行时进行的，从解析的时间上进行分类是<font color="#c00000">动态多态</font>。
