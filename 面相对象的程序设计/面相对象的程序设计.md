...
public:
    ...
    // 移动构造函数
    // 需要注意：这里仅对name做移动构造，对age做拷贝构造
    Cat(Cat&& cat) noexcept : name(std::move(cat.name)), age(cat.age)
    {
...
int main(int argc, char** argv) 
{
    ...
    std::cout << "The age of new_cat is: " << new_cat.get_age() << std::endl;
    ...
    std::cout << "The age of old cat is: " << cat1.get_age() << std::endl;
...
C++移动构造函数的特性有：
1. 当对象支持移动构造时，编译器会优先对函数返回值使用移动构造
2. <font color="#c00000">当对象满足如下条件时</font>，<font color="#c00000">编译器自动生成移动构造函数</font>：
	1. 用户未声明
	2. 所有非静态成员支持移动语义
3. 用户可通过 `Class(Class &&)=default;` 显示请求生成移动构造函数
