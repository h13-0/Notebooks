...
| `read`              | 可为空<sup>1</sup>                  |                     | 用于从设备中读取数据，返回值为读取的字节数。<br>在该函数的实现中，不需要驱动程序使用 `file->`                                                                                                                                                               |
...
| `flush`             | 可为空                              |                     | 含义同用户态的 `flush` 。如果该函数为NULL，则内核会忽略用户的 `flush` 请求。                                                                                                                                                                   |
| `release`           | 可为空                              |                     | <span style="background:#fff88f"><font color="#c00000">注意是没有close函数的，也就是该函数并不对应open函数</font></span>。<br>有时file文件在一次打开后会被共享(如使用了 `folk` 、 `dup` 等)，<br>则每个副本在用户态调用 `fclose` 后都会触发本函数。<br>该函数也是可以被设置为NULL的，效果与open相似。 |
