...
管道：
- 机制： ^0fhcgw
	- 管道是内核维护的一段缓冲区，一个进程写入，另一个进程读取(<span style="background:#fff88f"><font color="#c00000">单向传递</font></span>)
	- 管道传递的是字节流，不保留消息边界，一次写入的数据可能被多次读取
	- 常见种类有：
		- 匿名管道 `pipe` ，通常用于父子进程
...
- 机制： ^0x5zs3
	...
	- <span style="background:#fff88f"><font color="#c00000">消息队列支持双向数据传递</font></span>
	- 其<span style="background:#fff88f"><font color="#c00000">消息分条传递</font></span>，不过大小不能超过系统限制
	- 消息队列对队列中的消息总条数和消息总大小有限制，队满时入队会阻塞，队空时出队阻塞
	- 消息队列不要求收发双方的进程同时在线，其会先缓存于内核中
	- 常见种类和匹配方式有：
		- POSIX：文件描述符或名称匹配
		- System V：使用id进行匹配
