...
- 负责把分组从源端传送到目的端，为分组交换网上的不同主机提供通信服务。主要协议有<span style="background:#fff88f"><font color="#c00000">IP</font></span>、IPX、ICMP、IGMP、ARP、RARP、OSPF等。
- 传输单位是数据报。
- 主要功能有：
	- 路由选择：
	...
	- 差错控制：奇偶校验等
	- 拥塞控制：
		- 与路由选择的区别是，当所有节点都来不及接收分组而不得不丢弃大量分组的话，则此时网络就出于拥塞状态。而拥塞控制就是缓解这种拥塞(如全局降速等)。
- 网络层详见第四章。

<font color="#9bbb59">数据链路层</font>：
- 负责吧网络层传下来的数据报组装成帧，传输单位为帧。主要协议有<span style="background:#fff88f"><font color="#c00000">PPP</font></span>、SDLC、HDLC、STP。
- 主要功能：
	- 成帧(定义帧的开始和结束)。
...
其中：
- 第三层<span style="background:#fff88f"><font color="#c00000">数据链路层不止增加了头部</font></span>`H3`<span style="background:#fff88f"><font color="#c00000">，还增加了尾部</font></span>`L3`
- <font color="#c00000">路由器等中间系统至多涉足通信子网的内容</font>，其上部四层近似于直接连接，是<span style="background:#fff88f"><font color="#c00000">端到端</font></span>的通信；而下三层是<span style="background:#fff88f"><font color="#c00000">点到点</font></span>的通信。
#### 1.2.2.2 TCP/IP参考模型
