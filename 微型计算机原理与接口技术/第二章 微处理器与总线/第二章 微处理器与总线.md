...
内存分段技术最早就是由8086 CPU引入，由于其算术逻辑单元等寄存器均为16位的寄存器，但其想要达到20位的寻址空间(1MB)，因此其将20位的内存寻址空间划分为了64个逻辑段，并使用了 `CS` 、 `SS` 、 `DS` 、 `ES` 四个寄存器来进行指定给程序使用。
注意：
1. <span style="background:#fff88f"><font color="#c00000">逻辑地址的表示格式被规定为</font></span> `段地址:偏移地址` 。<font color="#c00000">段和段之间可以是连续的、分开的、部分重叠或完全重叠的</font>，由用户做冲突管理。
