...
<font color="#c00000">位填充是CAN总线的数据段和CRC段的数据填装方式</font>。<span style="background:#fff88f"><font color="#c00000">仅在这两个段使用</font></span>。
...
| <center>环节</center> | <center>数据示例1</center>                                                          | <center>数据示例2</center>                                                                                                                                                   |
| ------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
一个常见的CAN总线收发器的原理框图如下图所示：
	![[Pasted image 20241028224543.png]]
仅需注意如下几点：
1. <font color="#c00000">当CAN控制器使用TXD发送数据时，不影响RXD的接收</font>。
2. <font color="#c00000">CAN总线有"线与"特性</font>，总线上只要有一个设备发送逻辑"0"，则整个总线都表现为逻辑"0"。
3. <font color="#c00000">因此CAN控制器在发送数据的同时也在接收数据</font>，这个特性称之为"回读机制"。<font color="#c00000">但是由于线与特性，收到的数据可能与发送的不同</font>(总线仲裁章节中，非破坏性仲裁依赖的硬件特性)。
...
| 仲裁段 | 2. 替代远程请求(SRR)  | <font color="#c00000">逻辑"1"</font><br>与RTR电平不同 | 1   | 由于标准格式优先级大于拓展格式，<br>因此该位为1，原理可见章节"非破坏性仲裁"。<br> |
...
- 随后的8个错误界定符为逻辑"1"，<font color="#c00000">只有配合上后续的帧间隔才能保证连续的逻辑"1"大于等于11个</font>。<font color="#c00000">而连续11个逻辑"1"恰巧为总线空闲状态的判定条件</font>。
...
1. 要求发送方以固定时长输出一个数据位，接收方以固定时长接收一个数据位。
2. 接收方采样点位于数据位正中(但是具体可以进行调节)，例如：
	![[Pasted image 20241028192151.png]]
而CAN总线中一共给出了两种同步方式：
1. 硬件同步：<font color="#c00000">硬同步的核心思想是以第一个跳变沿为参考，对齐第一次的数据采样点</font>，<font color="#c00000">其用于避免起始时钟误差</font>。
2. 再同步：<font color="#c00000">再同步的核心思想是以正常数据位的跳变沿为参考，当发生微小偏差时，则缩短或延长单次的采样间隔</font>，<font color="#c00000">其用于解决时钟累计误差</font>(由于CAN总线报文通常很长，经常数百字节，所以要考虑时钟累计误差)。
这些同步方式将在位时序的学习后进行学习。
...
1. 同步段(SS)，固定位1Tq。<font color="#c00000">理想情况下SOF的跳变沿发生在此段</font>。
...
正如父章节所述，<font color="#c00000">硬同步的核心思想是以第一个跳变沿为参考，对齐第一次的数据采样点</font>，<font color="#c00000">其用于避免起始时钟误差</font>。
每一个CAN设备都有一个位时序计时周期，当接收方收到SOF的跳变沿时，其硬件会将计时周期拨回SS段。
<span style="background:#fff88f"><font color="#c00000">硬件同步仅在SOF的下降沿有效</font></span>。经过硬同步后，设备已经不存在起始时钟误差。

![[msedge_suqwKyUVFD.png]]
![[Pasted image 20241028221910.png]]
...
$波特率=\displaystyle\frac{1}{一个数据位的时长}=\frac{1}{T_{SS}+T_{PTS}+T_{PBS1}+T_{PBS2}}$

### 4.4 CAN总线仲裁

CAN总线是一个单工信号线，同一时间只能有一个设备的单向数据传输。当多个设备需要同时发送数据时，则就需要引入仲裁规则来保证数据的有序传输。
CAN总线仲裁的基本规则有如下两个：
1. 先占先得：先占用CAN总线的设备先使用总线。
2. 非破坏性仲裁：当两个报文同时传送时，报文ID小的设备先使用总线

#### 4.4.1 总线的基本状态

如上文CAN总线编码的若干规定，<span style="background:#fff88f"><font color="#c00000">CAN总线有如下的若干状态及其判定条件</font></span>：
...
<font color="#c00000">当且仅当设备判定到总线出于空闲状态时</font>，设备才可以使用CAN总线发送数据帧/遥控帧。<font color="#c00000">但是错误帧和过载帧不受此限制</font>。
...
<font color="#c00000">因此，当CAN总线同时传输两个报文的报文ID时，CAN总线总是表现出低地址的报文ID</font>。
这也是非破坏性的原理和特性。

#### 4.4.3 CAN总线仲裁所表现出的特性

在使用上述判定原则后，表现出如下特性：
1. <font color="#c00000">错误帧和过载帧优先级最高</font>，同时会破坏CAN总线上正在传递的数据。
2. <font color="#c00000">不同报文ID时</font>，低报文ID的报文优先级高(即使分别是数据帧和遥控帧，因为DTR位置在报文ID后)。
3. <font color="#c00000">同一报文ID的数据帧优先级大于遥控帧</font>，因为遥控帧DTR为逻辑"1"而数据帧为逻辑"0"。
4. <span style="background:#fff88f"><font color="#c00000">在标准格式的前11位ID与拓展格式的前11位ID一致时</font></span>，<font color="#c00000">标准格式的报文优先级比拓展格式的报文高</font>，因为拓展格式的两种报文的替代远程请求(SRR)恒为1，<span style="background:#fff88f"><font color="#c00000">且</font></span>ID扩展标志位(IDE)恒为1。注意：
	1. <span style="background:#fff88f"><font color="#c00000">当且仅当</font></span>前11位ID一致时，标准格式的报文优先级才会比拓展格式的报文高。
	2. 仅依靠SRR仅能判定前11位一致时，标准格式的数据帧大于拓展格式的所有帧。后续的IDE的判定才能完全保证上述特性。
