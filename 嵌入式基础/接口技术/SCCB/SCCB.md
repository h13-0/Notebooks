...
3. 每一个数据段包括9个bit位，其<font color="#c00000">前8个数据位表示1Byte数据</font>，且协议<font color="#c00000">不关心最后一个数据位</font>。而I2C中前8个表示1Byte，最后一个数据位为ACK(<font color="#c00000">强行兼容I2C</font>)；

## 2.3 SCCB地址与包结构

SCCB主要有两段或三段传输结构，其功能和定义见各子章节。
...
两阶段读即接收两阶段写指定的寄存器地址数据，其时序图与两阶段写一致，其各阶段含义为：
1. 设备地址(读写位<font color="#c00000">表示读取</font>)
2. 接收数据
总体与I2C保持一致。
