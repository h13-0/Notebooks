...
在I2C中，寄存器的读写位在最后一位；而在SPI中，<font color="#c00000">寄存器的读写位在第一位</font>。

##### 3.1.3.1 寄存器写入时序

在写入寄存器时，通常遵循如下的时序：
1. 使能CS引脚，并按照手册等待指定时长
2. 写入<font color="#c00000">标记了高位的</font>寄存器地址(为了保险期间，通常需要 `reg |= 0x80` )
3. 写入指定长度
...
在读取寄存器时，通常遵循如下的时序：
1. 使能CS引脚，并按照手册等待指定时长
2. 写入<font color="#c00000">标记了读取的</font>寄存器地址(为了保险期间，通常需要 `reg &= 0x7f` )
3. 读取指定长度
4. 失能CS引脚，并按照手册等待指定时长
