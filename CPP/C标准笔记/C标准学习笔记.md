...
**语义**
1. 会对每个操作数的类型进行提升，提升结果是运算符左侧操作数的类型。<span style="background:#fff88f"><font color="#c00000">如果右侧操作数的值是负值或超过了左侧操作数的宽度，则该结果是未定义行为</font></span>。
2. `E1 << E2` 的结果是 `E1` 左移 `E2` 位的位置，<font color="#c00000">空位用零填充</font>。如果 `E1` 是无符号类型，则结果的值为 $$E_1\times2^{E_2}$$，比结果类型中可表示的最大值多减少一个模。如果 E1有符号类型和非负值，并且 E1 × 2E2在结果类型中是可表示的，那么这就是结果值; 否则，行为是未定义的
