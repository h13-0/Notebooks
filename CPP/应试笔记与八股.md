...
具体对齐流程：
1. 将内存划分为与CPU单次存取内存的大小相同的内存块(在这里称作"<font color="#9bbb59">内存格</font>"，并非规范说法)，该"<font color="#9bbb59">内存格</font>"在默认情况下<font color="#c00000">仅受CPU位数影响</font>。
2. 找到结构体中单块最大的元素。
...
```
3. 对于 `gcc` 编译器，可以使用 `__attribute__((aligned (n)))` 来指定单个结构体的"<font color="#9bbb59">内存格</font>"。
```C
