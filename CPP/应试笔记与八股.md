...
| 15  | 赋值运算符               | `=` 、 `*=` 、 `/=` ...   | 右结合性                 |
| 16  | 逗号                  | `,`                     | 左结合性                 |
当然，C标准中并未给出上述规则，部分规则可能不严谨(例如经典++拔河题，例如使用该规则无法解释题目：[[CPP/应试笔记与八股#^1z06zt]])，<font color="#c00000">且未给出同优先级间的顺序关系</font>，但当应试时不得不处理该类题时可按照如下资料尝试求解。
...
### 2.19 [TODO]讲一讲Makefile



### 2.20 [TODO]讲一讲什么是do语句


### 2.21 [TODO]讲一讲常用的三种退出循环的方式

break、continue、goto的区别


### 2.22 C语言标识符(Identifiers)规则
...
### 2.31 对于一个n维数组a\[1\]\[2\]...\[n\]，则a\[1\]\[2\]...\[n-1\]是什么
...
### 2.37 [TODO]print函数和puts函数的区别



### 2.38 [TODO]讲一讲malloc、realloc、calloc的区别和用法


### 2.39 讲一讲字符串操作(strcmp、strcat、strcpy等)

C语言在 `<string.h>` 中主要实现了如下函数：

#### 2.39.1 `memcpy` 内存复制函数
...
### 2.40 [TODO]讲一讲存储类型(auto、static、extern、register)
...
### 2.47 为什么在C语言中，一定要用unsigned char表示byte而非signed char或char
