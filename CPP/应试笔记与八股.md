...
1. 多态在面向对象中主要是指"多种形态"，在C++中主要是指"一个接口的多种实现"
2. 从运行时/编译期可以分为静态多态和动态多态两种
3. 

## 3.13 讲一下C++中动态多态和静态多态之间的区别 ^5112yb

1. C++的动态多态是<font color="#c00000">基类和派生类之间的多态</font>，是垂直方向上的，是与类型相关的(例如用基类指针操作派生类的虚函数，那么调用的是基类版本)
2. 而C++的静态多态主要是<font color="#c00000">针对不同的数据类型，复用相同的逻辑代码</font>，不涉及继承关系。其主要设计如下两种类型：
	1. 函数重载：同名函数根据参数数量、类型的不同有不同的实现方式
	2. 模板：将函数实现模板化，自动根据传入的类型生成对应的代码
