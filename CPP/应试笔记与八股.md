...
![[Pasted image 20241025175924.png]]
...
- 数据段()，分为两段，<font color="#c00000">存放全局变量和静态变量</font>：
	- 未初始化数据段(.bss段)：<span style="background:#fff88f"><font color="#c00000">本段初值为0</font></span>，<font color="#c00000">存放定义时未指定具体值的变量</font>。
	- 已初始化数据段(.data段)：存放定义时指定具体值的变量。
- 文本段
- 代码段
