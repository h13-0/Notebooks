...
### 2.1.2 C语言执行效率高，简述一下C语言采用了哪些措施提高执行效率
...
### 2.1.7 讲一讲负整数在计算机中是如何存储的 ^p7e9a4
...
### 2.1.17 [TODO]讲一讲什么是do语句


### 2.1.18 [TODO]讲一讲常用的三种退出循环的方式

break、continue、goto的区别


### 2.1.19 对数组使用sizeof会得到什么结果 ^9r90wx
...
### 2.1.27 对于一个n维数组a\[1\]\[2\]...\[n\]，则a\[1\]\[2\]...\[n-1\]是什么
...
### 2.1.29 定义常量有哪些合法的方式(十六进制、科学计数法等)
...
### 2.4.15 为什么在C语言中，一定要用unsigned char表示byte而非signed char或char
...
### 2.2.2 [TODO]print函数和puts函数的区别



### 2.2.3 [TODO]讲一讲malloc、realloc、calloc的区别和用法

### 2.4.1 讲一讲字符串操作(strcmp、strcat、strcpy等)
...
## 2.10 void* 类型的指针作用是什么，相比于char* 、int* 等有什么优势？
...
### 2.4.7 [TODO]讲一讲存储类型(auto、static、extern、register)
...
## 2.4 [TODO]讲一讲Makefile



### 2.4.10 讲一讲C语言移位操作(`>>` and `<<`)
...
### 2.4.16 若Makefile中，头文件被修改，再次make会发生什么
...
1. free函数是C语言中用于释放 `*alloc` 所分配空间的函数。
2. 而C语言标准委员会说过， `*alloc` 系列函数的所有返回值必须可以被 `free` 函数接收，而 `*alloc` 有可能返回空指针，因此 `free` 函数可以接收 `NULL` 。

## 2.8 讲一讲sizeof发生在编译阶段还是运行时阶段

考察企业：
- 虹软 C++开发

## 2.9 在什么时候会在C语言头文件的函数声明前加extern
...
## 2.11 static和全局变量分别存储在哪里

考查企业：
- 禾塞科技 嵌入式

## 2.12 讲一下volatile关键字的作用
...
## 2.14 C的宏函数、内联函数和普通函数的区别是什么

考查企业：
- 影石 嵌软

## 2.15 讲一讲内存泄漏的排查工具及方法

考查企业：
- 影石 嵌软

1. 现代编译器通常已经内置运行时检测工具(例如AddressSanitizer)，编译时添加对应参数进行分析即可
...
## 3.7 在对象构造的过程中，由于异常等问题导致对象构造失败，那么析构函数能否被正常调用、已经成功动态创建的对象会发生什么

1. 当构造函数执行失败时，析构函数不会被调用，已经成功动态创建的对象也不会被自动释放
2. 可以考虑使用智能指针进行动态分配

## 3.8 对一个指针使用delete后，该指针是否会被置空
...
## 3.14 讲一下C++中动态多态和静态多态之间的区别 ^5112yb
...
## 3.17 讲一讲动态多态的实现原理，虚表和虚指针存放在哪里

考察企业：
- 中望 C++软开

## 3.18 讲一讲智能指针，为什么要有智能指针
...
## 3.21 share_ptr中哪些是线程安全的

考察企业：
- 理想汽车 系统软件开发

## 3.22 share_ptr的计数器存放在哪里
...
## 3.26 为什么要实现拷贝构造？讲一讲深拷贝和浅拷贝的区别

考察企业：
- 开立医疗 C++开发

## 3.27 返回值可以作为重载的区分条件吗

考察企业：
- 中望 C++软开

## 3.28 const和非const形参能作为重载的区分条件吗

考察企业：
- 中望 C++软开


## 3.29 引用计数有没有可能无法清零？如何解决循环引用的问题？

考察企业：
- 中望 C++软开
- 理想汽车 系统软件开发

## 3.30 STL中的sort函数使用的是什么排序

考察企业：
- 中望 C++软开

## 3.31 unordered_map和map有什么区别
...
## 3.32 迭代器的原理？为什么有了指针还要迭代器？

考察企业：
- 中望 C++软开

## 3.33 讲一讲vector和list的区别
...
## 3.36 讲一讲lambda表达式的原理

考查企业：
- 理想汽车 系统软件开发

## 3.37 lambda函数中捕获的变量存到了哪里

考察企业：
- 理想汽车 系统软件开发

## 3.38 讲一讲设计模式、以及什么是单例模式

考察企业：
- 旷视科技 自动驾驶部署优化


## 3.39 讲一下extern C及其作用
