...
2. C语言<span style="background:#fff88f"><font color="#c00000">并未规定</font></span> `printf` 函数<font color="#c00000">参数的计算顺序</font>，<span style="background:#fff88f"><font color="#c00000">只规定了从右向左依次入栈</font></span>(因为左边的要先出，所以只能后入)，但是<span style="background:#fff88f"><font color="#c00000">不得不做题时应当从右向左计算</font></span>。例如下列代码：
```C
int main()
{
    int a = 32;
