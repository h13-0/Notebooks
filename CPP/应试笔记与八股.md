...
### 1.16 [TODO]讲一讲Makefile



### 1.17 [TODO]讲一讲什么是do语句


### 1.18 [TODO]讲一讲常用的三种退出循环的方式

break、continue、goto的区别


### 1.19 C语言标识符(Identifiers)规则
...
### 1.32 print函数和puts函数的区别



### 1.33 讲一讲malloc、realloc、calloc的区别和用法


### 1.34 讲一讲字符串操作(strcmp、strcat、strcpy等)

C语言在 `<string.h>` 中主要实现了如下函数：

#### 1.34.1 `memcpy` 内存复制函数
...
### 1.35 [TODO]讲一讲存储类型(auto、static、extern、register)
...
2. <span style="background:#fff88f"><font color="#c00000">字节对齐的时候总会以结构体中单块最大的元素作为单位进行字节对齐</font></span>。
...
2. 可以使用宏 `#pragma pack (value)` 来控制该文件下所有结构体的"<font color="#9bbb59">内存格</font>"对齐单位。
```C
...
```
4. 对于 `gcc` 编译器，可以使用 `__attribute__((aligned(n)))` 来<span style="background:#fff88f"><font color="#c00000">扩大</font></span>一个结构体的"内存格"。例如对于下列<span style="background:#fff88f"><font color="#c00000">两个</font></span>代码片：
	- 代码片1：
		- 先用宏定义将该文件的所有结构体的"<font color="#9bbb59">内存格</font>"缩小为1
		- 再用本条所述指令将该结构体的"<font color="#9bbb59">内存格</font>"<font color="#c00000">重新</font><span style="background:#fff88f"><font color="#c00000">扩大</font></span>为2(不可用于)
		- 故总大小为6
```C
#pragma pack (1)  // 以8位为对齐单位
typedef struct {
...
// sizeof(type_a) = 6
typedef struct {
    uint16_t a;  // 0-2 Byte, a
    uint8_t b;   // 2-3 Byte, b
    uint8_t pad; // 3-4 Byte, pad, 被用于内存对齐
    uint16_t c;  // 4-6 Byte, c
} type_a;
