...
2. 基本ASCII表中共使用了128个编码，均位于低6位上，其中主要编码如下：
	1. `ASCII[0]` 到 `ASCII[31]` 是控制字符
	2. `ASCII[31]` 
	3. `ASCII[48]` 到 `ASCII[57]` 是数字 `0-9` ，<span style="background:#fff88f"><font color="#c00000">'0'在前面</font></span>
	4. `ASCII[65]` 到 `ASCII[90]` 是大写字符 `A-Z` ，大写排在


### 2.6 讲一讲负整数在计算机中是如何存储的
...
### 2.18 [TODO]讲一讲Makefile



### 2.19 [TODO]讲一讲什么是do语句


### 2.20 [TODO]讲一讲常用的三种退出循环的方式

break、continue、goto的区别


### 2.21 C语言标识符(Identifiers)规则
...
```
5. <span style="background:#fff88f"><font color="#c00000">但是不可缺失后面的维度</font></span>：
```C
...
### 2.34 print函数和puts函数的区别



### 2.35 讲一讲malloc、realloc、calloc的区别和用法


### 2.36 讲一讲字符串操作(strcmp、strcat、strcpy等)

C语言在 `<string.h>` 中主要实现了如下函数：

#### 2.36.1 `memcpy` 内存复制函数
...
### 2.37 [TODO]讲一讲存储类型(auto、static、extern、register)
