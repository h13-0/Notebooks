...
1. 数组在定义时会预分配好对应的静态空间，而指针则只是一个内存地址
2. 数组名的值代表数组对应内存空间的地址，但是数组名对应的值不可修改
3. `sizeof` 运算时：
	- `sizeof(ptr)` 只能获得对应平台下指针类型的大小，与实际内存空间无关
	- `sizeof(array)` 
...
## 2.33 对于一个n维数组a\[1\]\[2\]...\[n\]，则a\[1\]\[2\]...\[n-1\]是什么
...
## 2.39 [TODO]print函数和puts函数的区别



## 2.40 [TODO]讲一讲malloc、realloc、calloc的区别和用法

## 2.41 讲一下malloc是如何管理堆的

考查企业：
- 影石 嵌入式


## 2.42 讲一讲字符串操作(strcmp、strcat、strcpy等)
...
## 2.48 [TODO]讲一讲存储类型(auto、static、extern、register)
...
## 2.56 为什么在C语言中，一定要用unsigned char表示byte而非signed char或char
...
## 2.57 若Makefile中，头文件被修改，再次make会发生什么
...
## 2.64 void* 类型的指针作用是什么，相比于char* 、int* 等有什么优势？

考查企业：
- 影石 嵌入式


## 2.65 static和全局变量分别存储在哪里

考查企业：
- 禾塞科技 嵌入式

## 2.66 讲一下volatile关键字的作用
...
## 2.68 C的宏函数、内联函数和普通函数的区别是什么

考查企业：
- 影石 嵌软

## 2.69 讲一讲内存泄漏的排查工具及方法
