...
其中：
- `capture list` 为捕获列表，其中：
	- `[]` 表示不捕获任何外部变量
	- `[=]` 表示按值捕获函数体中所用到的外部变量
	- `[&]` 表示按引用捕获函数体中所用到的外部变量
	- `[v1, &v2]` 表示按值捕获变量 `v1` ，按引用捕获变量 `v2` 
	- `[=, &v]` 表示<font color="#c00000">除了</font> `v` <font color="#c00000">按引用捕获</font>，<font color="#c00000">其他按值捕获</font>
	此外需要注意：
	- `[=]` 与 `[&]` 不可混用
	- 不可重复捕获同一个变量：
...
`std::array` 在绝大多数特性上与C++的普通数组一致，其：
- 元素数量<span style="background:#fff88f"><font color="#c00000">必须在编译器即可确定</font></span>，<font color="#c00000">即也不支持动态数组大小</font>

但是其提供了如下的额外特性：
1. 避免了C语言数组函数参数传递时的[[CPP/应试笔记与八股#^tgxgrp|数组退化特性]]，即更安全的 `.size()` 机制
2. 作为函数参数传递时可以限制其尺寸
3. 提供了更安全的复制方法
