...
- `unsigned long flags`
	...
	- 可选性：<font color="#c00000">驱动设置初始标志</font>，V4L2运行时动态设置 `ENABLE` 位
- `bool is_backlink`
	- 功能含义：当该连接为反向连接时为 `true`
	- 可选性：V4L2框架自动管理

###### 3.1.5.1.3 媒体设备(media_device) ^elqnrp
...
###### 3.1.5.1.4 实体(media_entity) ^xup1xx
...
- `enum media_entity_type obj_type`
	- 功能含义：实体类型，V4L2框架仅定义了下述三种：
		...
		- `MEDIA_ENTITY_TYPE_V4L2_SUBDEV` ：嵌入在 `v4l2_subdev` 中的实体，即 `/dev/v4l-subdevX` ，例如：
			- 摄像头传感器
			- ISP
	- 可选性：<font color="#c00000">驱动必须指定</font>
- `u32 function` 
...
- `struct media_pad *pads`
	- 功能含义：焊盘所用的数组
	- 可选性：<font color="#c00000">驱动必须分配和维护对应内存区域</font>
- `struct list_head links`
- `const struct media_entity_operations *ops`
- `union info` 
