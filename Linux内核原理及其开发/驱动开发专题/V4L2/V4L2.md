...
##### 3.1.4.2 m2m设备操作回调(v4l2_m2m_ops) ^r39fw1
...
sequenceDiagram
    ...
    participant V4L2 as V4L2核心
    participant M2M as M2M框架
    participant Driver as 驱动回调
    participant HW as 硬件

    User->>V4L2: open("/dev/videoX")
    V4L2->>Driver: 创建v4l2_m2m_ctx
    User->>V4L2: VIDIOC_REQBUFS(OUTPUT)
    V4L2->>M2M: 初始化输出队列
    User->>V4L2: VIDIOC_REQBUFS(CAPTURE)
    V4L2->>M2M: 初始化捕获队列
    
    loop 处理循环
        User->>V4L2: VIDIOC_QBUF(OUTPUT)
        V4L2->>M2M: 输入缓冲区入队(用户态->v4l2)
        M2M->>M2M: 检查job_ready()
        alt 任务就绪
            M2M->>M2M: 添加任务到job_queue
            M2M->>M2M: 调度job_work
        end
        
        User->>V4L2: VIDIOC_QBUF(CAPTURE)
        V4L2->>M2M: 输出缓冲区入队(用户态处理过的缓冲区)
        M2M->>M2M: 检查job_ready()
        alt 任务就绪
            M2M->>M2M: 添加任务到job_queue
            M2M->>M2M: 调度job_work
        ...
        M2M->>Driver: 调用device_run()
        Driver->>HW: 配置并启动硬件
        HW->>Driver: 处理完成(中断)
        Driver->>M2M: v4l2_m2m_buf_done_and_job_finish()
        M2M->>User: 返回完成事件(DQEVENT)
    end
    
    User->>V4L2: VIDIOC_STREAMOFF
    V4L2->>Driver: 调用job_abort()
    Driver->>HW: 中止当前操作
    Driver->>M2M: 强制job_finish()
...
- `void (*device_run)(void *priv)`
	- 功能含义：
	- 可选性：<font color="#c00000">必须实现</font>
	- <span style="background:#fff88f"><font color="#c00000">关键规则</font></span>：
		- 该函数禁止阻塞
		- 任务完成后通过中断通知(异步)，需要调用 `v4l2_m2m_buf_done_and_job_finish`
- `int (*job_ready)(void *priv)`
- `void (*job_abort)(void *priv)`
