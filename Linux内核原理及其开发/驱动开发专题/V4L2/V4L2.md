...
sequenceDiagram
    ...
    participant V4L2 as V4L2框架
    participant Driver as 驱动回调
    participant HW as 硬件

    User->>V4L2: QBUF (入队缓冲区)
    V4L2->>V4L2: 检查 job_ready()
    alt 任务就绪
        V4L2->>Driver: 调用 device_run()
        Driver->>HW: 配置并启动硬件
        HW-->>Driver: 处理完成（中断）
        Driver->>V4L2: 调用 v4l2_m2m_buf_done_and_job_finish()
        V4L2->>User: 返回完成事件
    else 任务未就绪
        V4L2->>V4L2: 等待更多缓冲区
    end

    alt 中止请求 (close/STREAMOFF)
        User->>V4L2: 关闭流
        V4L2->>Driver: 调用 job_abort()
        Driver->>HW: 发送中止命令
        Driver->>V4L2: 强制 job_finish()
    end
```
