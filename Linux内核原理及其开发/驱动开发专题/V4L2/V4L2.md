...
| ------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `name`              |                       | 设备名称                                                                                                                                                         |
| `vfl_type`          | 驱动手动配置                | 常见设备类型举例(详见[[V4L2#^4ac1hk\|设备类型枚举]])：<br>- `VFL_TYPE_VIDEO` 视频输入输出设备(`/dev/video0`)<br>- `VFL_TYPE_RADIO` 无线电调谐器(`/dev/radiox`)<br>- `VFL_TYPE_VBI` 垂直消隐数据设备 |
| `vfl_dir`           |                       | 数据方向，`VFL_DIR_RX` 接收、`VFL_DIR_TX` 发送、`VFL_DIR_M2M` 内存到内存(常用于硬件编码器)                                                                                           |
| `flags`             |                       | 设备标志                                                                                                                                                         |
| `index`             |                       | 设备引索                                                                                                                                                         |
...
##### 3.1.3.1 设备类型(enum vfl_devnode_type)
