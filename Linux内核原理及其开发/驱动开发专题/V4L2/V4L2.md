...
需要注意：
1. <span style="background:#fff88f"><font color="#c00000">获得的缓冲区数量不一定等于申请到的缓冲区数量</font></span>。驱动会设定总缓冲区数量上限，并借此限制缓冲区数量上限(必然不会让用户态程序随便拉满缓冲区数量)。
...
如前文所述，[[mmap#^go6lxw|mmap]]<font color="#c00000">的操作对象是将一个fd的offset处映射到指定内存区域</font>。zai'zhe'li
