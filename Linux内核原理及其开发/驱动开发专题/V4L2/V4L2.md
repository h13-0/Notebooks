...
`video_device` 用于表示一个视频节点设备，例如 `/dev/video0` 
...
| <center>成员</center> | <center>配置方式</center> | <center>功能含义</center>                                                                                                                                                     |
| ------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `name`              |                       | 设备名称                                                                                                                                                                      |
| `vfl_type`          | 驱动手动配置                | 常见设备类型举例(详见[[V4L2#^4ac1hk\|设备类型枚举]])：<br>- `VFL_TYPE_VIDEO` 视频输入输出设备(`/dev/videox`)<br>- `VFL_TYPE_RADIO` 无线电调谐器(`/dev/radiox`)<br>- `VFL_TYPE_VBI` 垂直消隐数据设备(`/dev/vbix`) |
| `vfl_dir`           | 驱动手动配置                | 数据方向，`VFL_DIR_RX` 接收、`VFL_DIR_TX` 发送、`VFL_DIR_M2M` 内存到内存(常用于硬件编码器)                                                                                                        |
| `device_caps`       |                       |                                                                                                                                                                           |
| `flags`             |                       | 设备标志                                                                                                                                                                      |
| `index`             |                       | 设备引索                                                                                                                                                                      |
| `release`           | 驱动手动配置                | 设备释放回调                                                                                                                                                                    |
| `fops`              | 驱动手动设置                | 文件操作接口。该成员为 `v4l2_file_operations` 类型。<br>相比于普通的 `file_operations` 类型简化了相当多的成员(如 `flush` 、 `fsyns` 、 `read_iter` 等)                                                       |
| `dev`               | 框架自动配置                | Linux设备模型中的实例。                                                                                                                                                            |
| `cdev`              | 框架自动配置                | 处理用户空间对设备文件的文件操作。                                                                                                                                                         |
| `v4l2_dev`          |                       | 物理或逻辑上的V4L2主设备( `struct v4l2_device` )，<font color="#c00000">即指向所属的父级</font> ` v4l2_device ` <font color="#c00000">实例</font>。                                             |
| `dev_parent`        | 驱动按需配置                | 仅在 `v4l2_dev->dev` 无法正确反映物理父设备时手动设置。                                                                                                                                      |
| `queue`             | 驱动手动分配                | 视频缓冲区队列                                                                                                                                                                   |
| `tvnorms`           |                       |                                                                                                                                                                           |
| `v4l2_ctrl_handler` |                       | 用户可配置的参数项                                                                                                                                                                 |




##### 3.1.3.1 设备类型(enum vfl_devnode_type) ^4ac1hk
