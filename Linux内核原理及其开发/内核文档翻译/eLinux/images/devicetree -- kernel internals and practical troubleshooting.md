...
> 设备树是具有描述系统中设备的节点的<font color="#c00000">树形数据结构</font>。每个节点都有描述所表示设备特征的属性/值对。每个节点只有一个父节点，只有根节点没有父节点。 —— ePAPR v1.1
...
学习本章节前建议了解[[Linux内核原理及其开发/Linux驱动开发笔记#17 Linux设备模型 drcdil|Linux驱动开发笔记.Linux设备模型]]相关内容。

`of_platform_populate()` 函数只初始化部分平台设备，这些设备通常都是不可被主动发现(探测)的设备，例如：
- i2c设备
- SoC专用总线
等，这些设备通常都是由总线驱动程序的探测功能创建。
...
驱动程序匹配的优先级可见章节[[Linux内核原理及其开发/驱动开发专题/Linux设备模型/Linux设备模型#^9zo17a|驱动匹配规则与优先级]]：![[Linux内核原理及其开发/驱动开发专题/Linux设备模型/Linux设备模型#3 1 1 1 驱动匹配规则与优先级 9zo17a]]
