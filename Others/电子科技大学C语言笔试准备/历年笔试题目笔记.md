...
函数2：
1. 该函数的功能与函数1相同。
2. 该函数生成第 $i$ 个元素所需要生成随机数的次数的数学期望值为$$\frac{100}{100-i}, i\in[0, 99]$$ 故生成 $n$ 个随机数的生成次数数学期望约为 $$\sum_{i=1}^{n}{\frac{100}{100-i}}\approx\abobe n\cdot\ln(n)$$ 次，故时间复杂度为 $O(n\cdot\ln(n))$
3. 
