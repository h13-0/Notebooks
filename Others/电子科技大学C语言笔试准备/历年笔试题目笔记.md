...
#### 1.1 简述一下C语言采取了哪些措施提高运行效率(Finish)
...
#### 1.2 内存计算题(重要、多做)(Finish)

做P267的2015年第4题。

#### 1.3 函数结构与显示语言表示(Finish)
...
```
2. 使用子字符串搜索(使用 `strstr` 函数)，但是也没有简化边界条件
```C
int calculate_substr(char *A, char *B)
{
	int *start_pos = A;      //字符串A的起始搜索地址
	int *end_pos = A + strlen(A);   //找到字符串A的结束地址(`\0`字符地址)
    int len_B = strlen(B);   //计算字符串B的长度
    int substr_num = 0;      //A的子串数量
    if(lenA < len_B)
        return substr_num;
	while((start_pos < end_pos) && (strstr(start_pos, B)))
	{
		substr_num ++;
		start_pos += len_B;
	}
...
见2023第一题：
![[历年笔试题目笔记#1 1 简述一下C语言采取了哪些措施提高运行效率]]
...
原理：
	![[求质数方法#原理]]
TODO: 代码 https://leetcode.cn/problems/count-primes/solutions/507273/ji-shu-zhi-shu-by-leetcode-solution/
